...
function shim_gopath() {
  ...
  local TEMP_TASKGROUP="${TEMP_OSP}/manual-approval-gate"
  local NEEDS_MOVE=1
  ...
  # (repo-root)/.gopath/src/github.com/openshift-pipelines/manual-approval-gate
...
function delete_pipeline_repo_symlink() {
  ...
  local TEMP_GOPATH="${REPO_DIR}/.gopath"
  if [ -d "$TEMP_GOPATH" ] ; then
    local REPO_SYMLINK="${TEMP_GOPATH}/src/github.com/openshift-pipelines/manual-approval-gate"
