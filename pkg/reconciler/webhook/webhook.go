...
func (r *reconciler) Admit(ctx context.Context, request *admissionv1.AdmissionRequest) *admissionv1.AdmissionResponse {
	...
	if err != nil {
		...
		if CheckOtherUsersForInvalidChanges(oldObj.Spec.Approvers, newObj.Spec.Approvers, request) {
			...
			userApprovalChanged = false
			errMsg = fmt.Errorf("User %s is trying to modify other users' approvals or add unauthorized users", request.UserInfo.Username)
		}
	} else {
		userApprovalChanged = false
		errMsg = fmt.Errorf("User %s is not authorized to make this change or no valid change detected", request.UserInfo.Username)
	}
